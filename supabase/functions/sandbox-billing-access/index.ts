// GENERATED by scripts/build-sandbox-checkout.mjs from supabase/functions/billing-access/index.ts sha256:3cfe79e807dee681d584536a358d127183101cdb72f24a9ffe156eac9511c434
...
Deno.serve(async (req: Request) => {
  ...
  if (!/^cs_test_[A-Za-z0-9]{20,240}$/.test(body?.session_id || "") ||
  ...
  try {
    const {data: receipt, error} = await admin.from("billing_sandbox_checkout_access")
    ...
    const {data: locked, error: lockError} = await admin.from("billing_sandbox_checkout_access")
    ...
    const {error: saveError} = await admin.from("billing_sandbox_checkout_access")
