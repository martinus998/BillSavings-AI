// Supabase project security currently requires a symbol internally.
 // Users do not need to type one; new/reset passwords are canonicalized consistently.
 export function authStoredPassword(password) {
   return String(password ?? '') + '!Bs9';
 }
 export async function signInWithCompatiblePassword(supabase, email, password) {
   ...
   return supabase.auth.signInWithPassword({ email, password: authStoredPassword(password) });
 }
...
export function createPasswordAuth({
  ...
  function renderMode() {
    ...
    const confirmRequired = mode === 'recovery' || (mode === 'signup' && requirePasswordConfirmation);
    ...
    el.authConfirm.required = confirmRequired;
    el.authConfirmField.hidden = !confirmRequired;
    el.authConfirmField.classList.toggle('hidden', !confirmRequired);
  ...
  async function submit(event) {
    ...
    if ((mode === 'recovery' || (mode === 'signup' && requirePasswordConfirmation)) && password !== el.authConfirm.value) {
    ...
    try {
      ...
      if (submittedMode === 'recovery') {
        ...
        const { data: updated, error } = await supabase.auth.updateUser({ password: authStoredPassword(password) });
      ...
      const result = submittedMode === 'signup'
        ? await supabase.auth.signUp({ email, password: authStoredPassword(password), options: { emailRedirectTo: redirectTo } })
        : await signInWithCompatiblePassword(supabase, email, password);
      if (destroyed || requestGeneration !== generation) return;
