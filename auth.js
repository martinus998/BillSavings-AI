...
function addStyles() {
  ...
  s.id = 'bs-auth-styles';
  s.textContent = `
  .bs-modal{position:fixed;inset:0;z-index:99999;background:rgba(0,6,14,.78);backdrop-filter:blur(8px);display:none;align-items:center;justify-content:center;padding:18px}.bs-modal.show{display:flex}.bs-panel{width:min(520px,100%);max-height:90vh;overflow:auto;border:1px solid rgba(108,170,246,.30);border-radius:24px;padding:22px;background:linear-gradient(160deg,#071b35,#061324);box-shadow:0 30px 90px rgba(0,0,0,.55);color:#f4f8ff}.bs-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.bs-head h2{margin:0;font-size:25px}.bs-close{border:1px solid rgba(128,188,255,.25);background:#0b203b;color:#fff;width:38px;height:38px;border-radius:12px;font-size:20px;cursor:pointer}.bs-muted{color:#9fb8d4;font-size:13px;line-height:1.55}.bs-field{display:grid;gap:7px;margin:15px 0}.bs-field label{font-size:12px;font-weight:800;color:#cfe4fb}.bs-field input,.bs-field select{width:100%;padding:13px 14px;border-radius:13px;border:1px solid rgba(128,188,255,.25);background:#061427;color:#fff;outline:none}.bs-action{width:100%;padding:13px 16px;border:0;border-radius:13px;background:linear-gradient(135deg,#2f85ff,#3c6aff);color:#fff;font-weight:800;cursor:pointer}.bs-secondary{background:#0b203b;border:1px solid rgba(128,188,255,.25);margin-top:10px}.bs-status{margin-top:13px;padding:11px 12px;border-radius:12px;background:rgba(49,208,255,.08);border:1px solid rgba(49,208,255,.18);font-size:12px;line-height:1.45;color:#cfe9ff}.bs-status.err{background:rgba(255,92,92,.08);border-color:rgba(255,92,92,.22);color:#ffd4d4}.bs-account{display:none}.bs-account.show{display:block}.bs-upload{display:none}.bs-upload.show{display:block}.bs-userline{margin:10px 0 4px;font-size:13px;color:#cfe4fb}.bs-chip{display:inline-block;padding:4px 8px;border-radius:999px;background:rgba(30,220,171,.12);color:#6bf1c9;font-size:10px;font-weight:900;letter-spacing:.5px}.bs-note{font-size:10px;color:#86a6c4;line-height:1.45;margin-top:9px}`;
...
function buildModal() {
  ...
  modal.innerHTML = `
    <div class="bs-panel" role="dialog" aria-modal="true" aria-labelledby="bsTitle">
      <div class="bs-head"><div><div class="bs-chip">SECURE ACCOUNT</div><h2 id="bsTitle">BillSavings AI</h2><p class="bs-muted">Sign in with your email to securely upload and manage your own bills.</p></div><button class="bs-close" aria-label="Close">×</button></div>
      <div class="bs-login">
...
async function refreshUI(modal) {
  ...
  const account = $('.bs-account', modal);
  const upload = $('.bs-upload', modal);
...
function wireModal() {
  ...
  $('#bsUploadBtn', modal).addEventListener('click', async () => {
    ...
    if (rowError) {
      ...
      return setStatus(status, rowError.message, true);
    }

    $('#bsFile', modal).value = '';
    setStatus(status, 'Uploaded securely. Your file is private and attached to your account. Automated AI analysis is the next backend step.');
