...
function renderSession(user){if(currentUser?.id!==user?.id)clearAccountView();currentUser=user||null;$('loginBox').classList.toggle('hidden',!!user);$('signedBox').classList.toggle('show',!!user&&!recoveryMode);if(recoveryMode)$('loginBox').classList.remove('hidden');if(user)$('userEmail').textContent=user.email||'your account';configureAccountLayout();refreshPurchaseButton();}
